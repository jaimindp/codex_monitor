...
## Usage Screen (Codex model + cost tracking)

- The `Usage` screen now loads local telemetry from:
  - `~/.codex/state_5.sqlite` (`threads` table)
  - `~/.codex/sessions/**/*.jsonl` (model + token_count events when present)
- Data flow:
  - Renderer -> preload IPC (`codex-usage:get`, `codex-usage:refresh`) -> main process ingestion.
  - Main process stores a cached usage snapshot at Electron `userData/codex-usage-snapshot.json`.
- Usage cards + tables include:
  - Sessions tracked
  - Models tracked
  - Total tokens
  - Estimated cost (USD)
  - Per-model and recent-session rows
- Cost caveat:
  - The app labels this as an estimate from local telemetry.
  - Blended per-model rates are loaded from `userData/codex-pricing-settings.json`.
  - Default rates are placeholders and should be replaced with your real rates.
